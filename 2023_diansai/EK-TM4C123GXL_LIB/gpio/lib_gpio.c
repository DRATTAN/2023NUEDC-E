...
void LIB_GPIO_Init(uint32_t Port, uint8_t Pin, uint8_t Dir, uint32_t Type, uint8_t Value)
{
    ...
    else if(GPIO_DIR_INPUT == Dir)
    {
        ...
        GPIOPadConfigSet(Port, Pin, GPIO_STRENGTH_4MA, Type);
    }
    else return ;
}
...
void LIB_GPIO_ExtiInit(uint8_t ISRGPIOPort, uint8_t ISRGPIOPin, uint8_t Trigger, void (*ISR_handler)(void))
{
    ...
    else if(Trigger == GPIO_EXTI_TRIGGER_FALLING || Trigger == GPIO_EXTI_TRIGGER_LOW || Trigger == GPIO_EXTI_TRIGGER_BOTHEDGES) LIB_GPIO_Init(GPIO_BASE_TABLE[ISRGPIOPort], 0x01 << ISRGPIOPin, GPIO_DIR_INPUT, GPIO_PIN_TYPE_STD_WPU, 0x00);
