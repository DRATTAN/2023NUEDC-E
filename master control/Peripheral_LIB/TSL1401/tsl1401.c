...
volatile uint16_t exp_time = 300;
void Tsl1401_Init()
{
...
void Tsl1401_GetData(uint8_t *buffer)
{
    CLK_WRITE(1);
    SI_WRITE(0);
    delay_us(1);
...
int16_t Ts1401_Trans(uint8_t* dpoint)
{
    int16_t maxret = 0;
    ...
    for(uint8_t i = 6   ; i < 126; i += 5)
    {
        buffer[i] = (int16_t)dpoint[i] - (int16_t)dpoint[i - 5];
    }
...
uint16_t Tsl1401_AutoSetExpTime(uint16_t max, uint8_t is_information)
{
    int16_t maxdef = 0;
    uint16_t max_exp = 10;
    int16_t def = 0;
    uint8_t buffer[128];
    exp_time = 10;
    if(is_information) Oled_ShowString(0, 0, "WATTING");
    while(1)
    {
        Tsl1401_GetData(buffer);

        def = Ts1401_Trans(buffer);
        if(exp_time > max)
        {
            exp_time = max_exp;
            return max_exp;
